...
parser.add_argument('--data', default='../data/CUB_200_2011/',
...
def train(train_loader, model, old_model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        class_output = old_model.fc(feature)
        old_feature = old_model(input)
        class_output_target = old_model.fc(old_feature).detach()


        loss_rot = criterion(rot_output, target)
        loss_KD = loss_fn_kd(class_output, class_output_target, T=2.)
        loss = loss_rot + loss_KD*0.01
