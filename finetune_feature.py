...
parser.add_argument('--data', default='../../../CUB_200_2011/',
...
parser.add_argument('--gpu', default=0, type=int,
                    help='device id')
parser.add_argument('--coef', default=0.05, type=float,
                    help='loss coefficient')
parser.add_argument('--pretrained_model', default=None, type=str,
                    help='pertrained_model path')
best_prec1 = 0
...
def main():
    ...
    loading_dict = torch.load(args.pretrained_model, map_location=torch.device('cuda:'+str(args.gpu)))['state_dict']
    feature.load_state_dict(loading_dict, strict=False)
    print('feature weight loaded')      
    fc = nn.Linear(int(256*(256+1)/2), args.num_classes)
    model = nn.Sequential(feature, representation, nn.Flatten(1,2), fc)
    print('************************************************')
    print(model)
    print('************************************************')
    for p in model[:3].parameters():
        p.requires_grad = False
    freeze_flag = True 
    print('fixed model 0 1 2 ')


        
    #old_model 
    old_model = mpncovresnet50_pre()
    old_model.load_state_dict(torch.load('mpncovresnet50_img.pth', map_location=torch.device('cuda:'+str(args.gpu))))
    for para in old_model.parameters():
        para.requires_grad = False
    print('old model fixed')    
    print('************************************************')
    print('old_model loaded')
    print('************************************************')
    print(old_model)
    
    LR = Learning_rate_generater(args.lr_method, args.lr_params, args.epochs)
    params_list = [{'params': filter(lambda p: p.requires_grad, model[0].parameters()), 'lr': args.lr,
                        'weight_decay': args.weight_decay},]
    params_list.append({'params': filter(lambda p: p.requires_grad, model[1].parameters()), 'lr': args.lr,
                        'weight_decay': args.weight_decay})
    params_list.append({'params': filter(lambda p: p.requires_grad, model[3].parameters()),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch > int(args.epochs // 2) and freeze_flag:
        ...
        # train for one epoch
        trainObj, top1, top5 = train(train_loader, model, old_model, criterion, optimizer, epoch)
...
def train(train_loader, model, old_model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        feature = model[:3](input)   
        output = model[3](feature)

        feature_old = old_model(input).detach()

        loss_class = criterion(output, target)
        loss_feature = torch.norm(feature_old-feature, p=1, dim=1).mean()
        loss = loss_class + loss_feature*args.coef
...
def loss_fn_kd(scores, target_scores, T=2.):
    ...
    log_scores_norm = nnF.log_softmax(scores / T, dim=1)
    targets_norm = nnF.softmax(target_scores / T, dim=1)
