...
parser.add_argument('--data', default='../../../Aircraft/',
...
def train(train_loader, model, old_model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        feature_old = old_model(input)
        target_score = old_model.fc(feature_old)
        soft_logit_old = old_model.fc(feature)
        target_score = target_score.detach()

        loss_class = criterion(output, target)
        loss_KD = loss_fn_kd(soft_logit_old, target_score, T=2)
        loss = loss_class + loss_KD*args.coef
