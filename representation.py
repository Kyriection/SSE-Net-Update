...
class MPNCOV(nn.Module):
     ...
     """
     def __init__(self, iterNum=3, is_sqrt=True, is_vec=True, input_dim=2048, dimension_reduction=None, cat=False):
         ...
         if self.is_vec:
             self.output_dim = int(output_dim*(output_dim+1)/2) #+ int(64 * 65 / 2) * (num_branches - 1)
         else:
             self.output_dim = int(output_dim*num_branches)
         self._init_weight()
