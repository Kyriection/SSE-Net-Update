...
def main():
    ...
    #ssp model 
    model = mpncovresnet50_pre()
    model.load_state_dict(torch.load('mpncovresnet50_img.pth', map_location=torch.device('cuda:'+str(args.gpu))))
    model.rot = nn.Linear(int(256*(256+1)/2), 4)
    model.cuda()
    print('************************************************')
    print('model loaded')
    print('************************************************')
    print(model)
    #warm up strategy: for the First half of training phase, only fc layer can be trained
    for para in model.parameters():
        para.requires_grad = False
    for para in model.rot.parameters():
    ...
    params_list = [{'params': filter(lambda p: p.requires_grad, model.parameters()), 'lr': args.lr,
...
def train(train_loader, model, old_model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #print(target.shape)
        feature = model(input)  
        rot_output = model.rot(feature)

        old_feature = old_model(input).detach()

        loss_rot = criterion(rot_output, target)
        loss_feature = torch.norm(old_feature-feature, p=1, dim=1).mean()
        loss = loss_rot + loss_feature*args.coef 

        # measure accuracy and record loss
        prec1, prec5 = accuracy(rot_output, target, topk=(1, 3))
